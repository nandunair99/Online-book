...
public class InsertUsers extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			UserValidator.validate(userDTO,addressDTO);
			if( Integer.parseInt(type) == Constant.USER_TYPE_SUPERCINEMAADMIN)
			{
				if (validationFunction.isNullOrEmpty(cinemaID)) {
					
					throw new ValidationException("validErrorCinema", "Please select cinema!");
				}
			}
		...
		if (profile.getSize() != 0) {
			...
			File defaultProfile =  new File(request.getServletContext().getRealPath("/") + "resources/img/avatars/user.png");
		...
		try {
			if( Integer.parseInt(type) == Constant.USER_TYPE_SUPERCINEMAADMIN)
			{
				UsersDAO.insertUser(user,address,Integer.parseInt(cinemaID));	
			}else
			{
				UsersDAO.insertUser(user,address);
			}

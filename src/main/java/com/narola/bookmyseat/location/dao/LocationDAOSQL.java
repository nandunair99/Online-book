...
public class LocationDAOSQL implements ILocationDAO {
	// Insert State
	 ...
	 * 
	 * @param txtstate State name
	 * @return Return a all data of state which name is match to parameter name.
	 * @throws DBException Throw a error if database related issue occur.
	 */
	...
	public int insertCity(City city) throws DBException {
		...
		try {
			Connection cn = DBConnector.getInstance().getConnection();
			ps = cn.prepareStatement("insert into tblcity (name,state_ID,createdTime,updatedTime) values (?,?,?,?)",
					Statement.RETURN_GENERATED_KEYS);

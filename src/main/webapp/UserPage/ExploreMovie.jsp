			...
			<div class="bannerBox" style="background-image: url(data:image/png;base64,${imgBannerById}); background-repeat: no-repeat; background-size: cover ">
    			...
    			<div class="movieData row">
    				...
    				<div class="col-9 dataContains">
						...
						<c:if test="${movieDataById.getStatus() == 0}">
							<button style="padding: 0.3rem 2.5rem;" class="btn btn-success">Book tickets</button>
						</c:if>
						<c:if test="${movieDataById.getStatus() == 2}">
							<button style="padding: 0.3rem 2.5rem;" class="btn btn-secondary">I'm Interested</button>
						</c:if>
    				</div>
    			</div>
			</div>
